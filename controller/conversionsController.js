...
module.exports = {
  createChatRoom: async (req, res) => {
    ...
    if (user_two && user_one && lastMessage) {
      ...
      let chat = await roomsSchemaModel.findOne({
        users: { $all: [user_one, user_two] },
      });

      if (chat === null) {
        //create new Conversation
        const roomModel = new roomsSchemaModel({
      ...
      res.send({
        error: true,
        data: "missing some args user_one user_two lastMessage",
      });
    }
  },
  getUserChats: async (req, res) => {
    ...
    if (userId) {
      ...
      let result = {};
      try {
        let chats = await roomsSchemaModel
          .find({ users: userId })
          .sort({ updatedAt: -1 })
          .populate("users", "_id img name token");
        result.error = false;
